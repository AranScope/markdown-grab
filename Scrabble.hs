...
numOcc c cs = length $ filter (==c) cs
...
formable s1 s2 i = submultiset s1 (i:s2)
...
toppadding board = minimum $ [length x | x <- map (takeWhile (==Nothing)) board]
-- leftpadding board = length [x | x <- board, and $ map (==Nothing) x]

addtoppadding n board = [replicate n Nothing ++ b | b <- board] 
-- addleftpadding n board = (replicate n (replicate (length $ head board) Nothing)) ++ board

-- autoResize' :: Board -> Board
-- autoResize' board = addtoppadding (7 - (toppadding board)) $ addleftpadding (7 - (leftpadding board)) board

fixtoppadding :: Board -> Board
fixtoppadding board = addtoppadding (7 - (toppadding board)) board

rotate :: Board -> Board
rotate board = map reverse $ transpose board 

autoResize :: Board -> Board
autoResize board = fixtoppadding $ rotate $ fixtoppadding $ rotate $ fixtoppadding $ rotate $ fixtoppadding $ rotate board 
...
-- the fst of move is the string to be placed
-- the snd of move is ((int, int), H | V)
-- where (int, int) = (x, y) 

writeMove' :: Move -> Board -> Board
writeMove' ("", _) board = board
writeMove' ((s:ss), wordpos) board = undefined
...
replenishRack rack = sequence ( (map return rack) ++ (replicate (rackSize-length(rack)) newLetter) )

-- replenishRack rack = if (length rack) == 7 then rack else (replenishRack (newLetter : rack))
--replenishRack rack = let Bram.LRand f = Bram.replenishRack rack in LRand f

-- replenishRack rack = ([pickUniformly [x] | x <- rack]) ++ ([newLetter | count <- [1..(7 - length(rack))]])

randomRack = [newLetter | count <- [1..7]] 
...
allWords1 dict c = [word | word <- dict, c `elem` word]
...
-- elemIndices char word

-- we basically want every word from allwords1
-- calculate the crossover of each word and return
-- the set of words and crossovers

allWords2 :: Dict -> Char -> Int -> Int -> [(String, Int)]
allWords2 dict char lspace rspace = [(word, (index+1)) | word <- (allWords1 dict char), index <- elemIndices char word, index <= lspace, rspace >= (length word) - index - 1] 

-- we want to filter based on words that first contain the character (using allwords 1)
-- then we need to check if the index of the given character 
...
allWords3 dict rack char lspace rspace = filter (\w -> (submultiset (fst w) (char:rack)) /= Nothing) (allWords2 dict char lspace rspace) 
...
-- Scrabble.FullMove.
--greedyBestMove dict rack board = maximumBy $ (compare `on` moveScore) (allMoves dict rack board)
greedyBestMove dict rack board = maximumBy (compare `on` moveScore) (allMoves dict rack board)
